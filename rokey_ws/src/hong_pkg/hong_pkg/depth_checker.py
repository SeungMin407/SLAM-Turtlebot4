...
class DepthChecker(Node):
    def __init__(self):
        ...
        self.K = None               # camerainfo에서 오는 3x3 intrinsic matrix.
    ...
    def depth_callback(self, msg):
        ...
        depth_mm = self.bridge.imgmsg_to_cv2(msg, desired_encoding='passthrough') # 여기서 깊이 뽑아줌!
        height, width = depth_mm.shape

        cx = self.K[0, 2]
        cy = self.K[1, 2]
        u, v = int(cx), int(cy)  # cy, cx : 이미지 중심 근처 principal point
