...
class DepthToMap(Node):
    ...
    ### yolo 받아오면 실행 ###
    def yolo_center_callback(self, msg: Float32MultiArray):
        if msg.data is None or len(msg.data) < 2: #메시지가 없으면 하지마
            return
        cx = float(msg.data[0])
        cy = float(msg.data[1])

        # 락건 상태로, clicked point 정의 -> display image()가 읽어서.
        with self.lock:
            self.clicked_point = (cx,cy)
        ...
        if rgb is not None and depth is not None and frame_id:
            try:
                ...
                if click is not None:
                    ...
                    x = int(round(x_f)) 
                    y = int(round(y_f))
                    z = float(depth[y, x]) / 1000.0  # depth는 int로 받아야함.
                    if 0.2 < z < 5.0:
